...
- [x] **P6-T01** — `uaaf/execution/pool.py`: AgentPool (register, dispatch round_robin/random, dispatch_to, fan_out fail_fast/collect via anyio)
...
- [x] **P6-T03** — `uaaf/cognitive/strategies/parallel.py`: ISubtaskBuilder Protocol + EntitySubtaskBuilder + ParallelFanoutStrategy (ICognitiveStrategy #4, dùng fan_out on_error="collect")
...
- [x] **P6-T07** — `uaaf/_testing/fakes.py`: add `FakeAgentPool.fan_out(tasks, context, on_error)` method
  - Gate: `pytest tests/unit/_testing/` → pass

- [x] **P6-T08** — CHANGELOG v0.1.0b6 + `uaaf/__init__.py` exports + project memory
...
pytest tests/ --cov=uaaf --cov-fail-under=88           → 492 pass, 88.02% coverage ✅
...
| File | Type | Est. Lines |
|------|------|-----------|
| `uaaf/execution/pool.py` | new | ~130 |
| `uaaf/cognitive/strategies/parallel.py` | new | ~125 |
...
| `uaaf/cognitive/strategies/__init__.py` | modify | +6 exports |
| `uaaf/__init__.py` | modify | +1 export, version bump |
