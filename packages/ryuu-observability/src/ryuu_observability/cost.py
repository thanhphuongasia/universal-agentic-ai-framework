...
from ryuu_core.models import Cost as Cost  # noqa: F401 — re-export for backward-compat
...
class CostTracker:
    ...
    def enforce(self, scope_key: str, estimated: Cost) -> None:
        ...
        if self.policy.per_user_per_day_usd is not None and projected > self.policy.per_user_per_day_usd:
            raise BudgetExceededError(
                f"Scope {scope_key!r}: projected ${projected:.4f} exceeds "
                f"per-user-per-day cap ${self.policy.per_user_per_day_usd:.4f}"
            )
        if self.policy.per_domain_per_month_usd is not None and projected > self.policy.per_domain_per_month_usd:
            raise BudgetExceededError(
                f"Scope {scope_key!r}: projected ${projected:.4f} exceeds "
                f"per-domain-per-month cap ${self.policy.per_domain_per_month_usd:.4f}"
            )
        if self.policy.global_per_hour_usd is not None and projected > self.policy.global_per_hour_usd:
            raise BudgetExceededError(
                f"Scope {scope_key!r}: projected ${projected:.4f} exceeds "
