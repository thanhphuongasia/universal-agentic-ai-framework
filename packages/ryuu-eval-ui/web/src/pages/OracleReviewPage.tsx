...
function Tab3_Execution({
  ...
  const hasCells = !!source && typeof source === "object" && Object.keys(source).length > 0;
  // Pluggable shape renderer — CellsView dispatches to crud/list/flat/json
  const detectedShape = pickRenderer(source).shape;
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-gray-500 dark:text-gray-400">
        {hasCells
          ? <>{preview ? "Preview" : "Saved expectation"} — rendered as <span className="font-mono font-semibold">{detectedShape}</span> shape</>
          : preview
            ? (preview.parse_error
