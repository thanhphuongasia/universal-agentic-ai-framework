...
Override the file path via env var: RYUU_PRICING_FILE=/path/to/pricing.yaml
...
def _resolve_pricing_file() -> Path:
    ...
    env = os.getenv("RYUU_PRICING_FILE")
    if env:
        return Path(env)
    # 2. Co-located pricing.yaml — works for both installed package and source checkout
