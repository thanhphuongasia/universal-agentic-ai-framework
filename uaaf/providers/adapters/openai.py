"""OpenAI ILLMProvider adapter — T09.

Wraps ``openai.AsyncOpenAI`` and maps its responses/errors to UAAF types.
Install with: ``pip install "uaaf[openai]"``
"""
...
class OpenAIProvider:
    ...
    def __init__(
        ...
        **client_kwargs: Any,
        ...
        if AsyncOpenAI is None:  # pragma: no cover
            raise ImportError('Install openai: pip install "uaaf[openai]"')
    ...
    async def complete(self, request: CompletionRequest) -> Response:
        ...
        # Expose tool_calls so agents can implement function-calling loops.
        import json as _json
    ...
    def estimate_cost(self, request: CompletionRequest) -> Cost:
        model = request.model or self._default_model
        # Rough estimate: max_tokens is output upper bound; prompt length as input proxy.
